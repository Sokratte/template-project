...
- **Line-1 header changed to a single self-identifying line (session 032):** the old two-line header (`<!-- keywords: … -->` on line 1, bare filename on line 2) is retired ersatzlos in favour of `<!-- file: <filename> · keywords: … -->` on line 1. The filename now lives inside the keyword comment, so one harvested `head -1` line is self-identifying and the directory index works without an exec-run loop — or for an agent given no exec at all. The previous design only mapped path→keywords when an exec loop explicitly emitted the path. `file:` is the bare basename where unique, repo-relative path where not (the 9 READMEs). Migrated all 49 `docs/` files (ADR-001/002, DP-000, SPEC-001/002, PLAN-001/002/003, all research, all 31 session logs — header only, narrative untouched) plus `src/README.md` and `tools/README.md`. Rule rewritten in `docs/README.md` (all 6 templates updated, basename-vs-path rule documented); `session-start.md`, `session-end.md`, and PLAN-003 prose updated. No ADR — the convention was never ADR-ratified; its home is `docs/README.md`. `agents/` and `VM.local/` use H1 headers and were not affected.
