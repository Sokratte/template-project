...
- **TEST document type + doc-consistency suite (session 031):** added an executable suite `tests/test_docs.py` (4 checks: no legacy `kw:` header token, no pipe-form headers, content-doc line-1 keywords-comment + bare filename, no dead ADR/SPEC/PLAN/DP references) that reads the live tree and excludes `.trash/`. Introduced the **TEST** document type (`docs/tests/TEST-NNN-*.md`) that registers a suite without containing it; registered in `docs/README.md` (types table, status subset Draft·Active·Superseded, template). Added `docs/tests/TEST-001-doc-consistency.md` and `docs/tests/README.md`. Wired a doc-consistency gate into `session-end.md` (new step 7, before commit). Suite found and fixed two pre-existing defects: session-030 pipe-form header, session-028 wrong filename anchor. Plan: PLAN-003.
