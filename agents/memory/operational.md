...
- Sandbox tools (`str_replace`, `create_file`, `view`, `bash_tool`) operate on the agent sandbox, NOT the operator's real filesystem — they fail or silently mislead on project files. Use the filesystem MCP tools (`filesystem:read_text_file`, `filesystem:write_file`, etc.) for all project work. `bash_tool` also cannot reach the real FS, so no grep/git from there. [2026-06-20 x3]
