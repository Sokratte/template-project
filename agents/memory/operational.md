...
## Tooling · keywords: macbook-mcp, maisig-auditor scope, python heredoc edits, str.replace assert count, python3 stdin heredoc, python3 dash heredoc, write_file timeout hang, tool_search narrow query hides exec
...
- `python3 - << 'PYEOF'` (dash form, stdin) DOES run reliably in `macbook-mcp:exec` — used repeatedly this session for multi-block `str.replace` edits. The s27 failure was specific to the `/dev/stdin` path form, not heredoc-to-python as such; prefer `python3 - <<` over the tmp.py dance. [s30 x2]
- `macbook-mcp:write_file` (and occasionally `:exec`) can hang ~4 min and return "unresponsive"; the write may or may not have landed. Fall back to an `exec` heredoc (`cat > f << 'EOF'`), and always re-check the FS state before retrying so you neither double-write nor assume a no-op. [s30 x2]
- `macbook-mcp` exposes `exec`/`git`/`run_pytest`/`search` beyond the read/write wrappers, but a narrow `tool_search` query (e.g. "filesystem") returns ONLY read/write — concluding from that partial result that exec is absent is a real error made this session, and it drove a needless full-file rewrite. Load the full toolset (broad query, limit ~20) before deciding a capability is missing. [s32 x1]
