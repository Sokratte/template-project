...
3. **Prepare the session log scaffold.** Create `docs/sessions/YYYY-MM-DD-NNN-<topic>.md`. Line 1: `<!-- file: <filename> · keywords: … -->` — the single self-identifying header (see `docs/README.md`).
4. **Build the startup index.** Read line 1 (`<!-- file: … · keywords: … -->`) of every doc — each line carries its own filename, so the index is self-identifying. This is the last step so it sits at the recency end of the startup load, just before the operator's first instruction:
   ```bash
   for f in \
     docs/decisions/*.md \
     docs/specs/*.md \
