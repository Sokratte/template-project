...
3. **Session log** — `docs/sessions/YYYY-MM-DD-<topic>.md`
   Fill the scaffold from session start: done, found, decided, open. Line 1 is the greppable, self-identifying index entry: `<!-- file: <filename> · keywords: … -->`. If work is unfinished, add a "State at close" section. Extract durable findings (specs, operational memory, research) and leave a pointer rather than burying them here.
