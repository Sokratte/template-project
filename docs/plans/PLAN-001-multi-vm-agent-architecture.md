...
Cutting across both axes is a third thing that belongs to neither: **who the operator is and what this machine is like.** That is not instructions (it is not synced/identical across VMs) and not project context (it is not project-scoped and must never be committed). It gets its own file, `OPERATOR.md`, at the workspace root.
...
There are no longer any `.project` / `.projects` config files. Backup remote config lives in `.git/config` (git's own); operator/VM facts live in `OPERATOR.md`; the two per-project toggles live in `AGENTS.override.md`.
...
- **Identical across all VMs.** This is the invariant the whole design protects. Nothing VM-specific or operator-specific goes here — that is what `OPERATOR.md` is for.
...
- A cloner is a different operator on a different machine → they get their own `OPERATOR.md`; inheriting the original operator's would be both wrong and a privacy leak.
...
- **Note:** `agents/memory/procedural.md` now holds procedural RULES only — the operator-profile section moved to `OPERATOR.md` (Decision 11).

### 9. SSH keys — `~/.ssh/<project>[.pub]`

- Private + public deploy key per repo. **Outside every project tree**, never committed, `600` perms. Path is **derived by convention** from the project name (documented in `OPERATOR.md`), not stored anywhere.
...
- **Cloner with the full workspace:** their own `agents_sync.sh` + canonical file sync their version in. They write their own `OPERATOR.md`.
- **Cloner of a single project repo, no workspace:** no script exists to run, so nothing overwrites anything. The committed in-repo `AGENTS.md` stands as the working fallback. No `OPERATOR.md` is present (correct — different operator), so the agent prompts to create one.
...
2. **Exec call 1 — sync, read identity, list projects, orient** (one call; the script self-enumerates, so it needs nothing from `AGENTS.md` first):
   ...
   ```
   Project list is **derived from folders, never stored.** `OPERATOR.md` may be absent (fresh clone, new VM) → the agent prompts to create one.
...
- `OPERATOR.md` says `backup: none` → `git init`, auto-commit at session end, **no remote, no push, git never mentioned to the user.**
- `OPERATOR.md` says `backup: <provider(s)>` AND the project override has `push: on` AND `origin` is set → auto-commit AND `git push`, gated by `autonomy`.
...
1. **Root:** `~/projects/`. Canonical `AGENTS.md`, `OPERATOR.md`, and the two scripts live there.
...
4. **Deploy keys per repo** are the default (security); path derived by convention `~/.ssh/<project>`, documented in `OPERATOR.md`. Keys live in `~/.ssh/`, never in the tree.
...
10. **`OPERATOR.md` (was `.projects`) is operator+VM identity, Markdown, NEVER committed, NEVER cloned.** Rationale: privacy (operator profile is PII) + it is the per-VM seam that keeps canonical `AGENTS.md` identical.
11. **Operator profile has ONE home: `OPERATOR.md`.** It is REMOVED from `agents/memory/procedural.md`, which becomes procedural-rules-only.
...
- **`~/projects/` is NOT a git repo.** Canonical `AGENTS.md`, `OPERATOR.md`, and the scripts are maintained by hand and backed up by the operator's own means.
...
- **`.project` / `.projects` eliminated; `OPERATOR.md` introduced; operator profile de-duplicated to a single non-committed home.**

## Execution checklist

- [x] Canonical `AGENTS.md` in `~/projects/`, identical across VMs, the only hand-edited copy, under line budget
- [x] `OPERATOR.md` in `~/projects/`: operator profile + backup pref + setup habits + key convention; never committed
- [x] Operator profile REMOVED from `procedural.md` (single home in OPERATOR.md)
...
- [x] Git behavior keyed on `.git/config` + `OPERATOR.md backup` + override `push`; `none` path never mentions git
...
- [ ] Cloner scenario (single-repo clone, no workspace) verified: committed AGENTS.md serves as fallback, agent prompts for OPERATOR.md
